...
Last activity: 2026-09-08 - Completed quick task 260908-l5m: run bench under plain Node via an out-of-process ABI swap wrapper
...
| 260715-4np | Inject OAuth secrets into CI `Build (vite)` step (build-win + build-mac) — `.env.local` gitignored/absent in CI so `loadEnv` baked "" → OAuthConfigMissingError on prod builds; now sourced from GitHub repo secrets (GOOGLE/MS_OAUTH). YAML parse + both-jobs env-key checks pass; Build (vite) step confirmed GREEN in CI run 29403002015 | 2026-07-15 | 9fe96a5 | Needs Review |
| 260715-pkg | Unblock CI packaging (broken since 2026-06-28, blocking OAuth UAT installer). electron-builder 26 rejected invalid `platform` key on 2 whisper extraResources entries at schema validation. Relocated whisper to win/mac extraResources blocks; added "Stage voice binaries dir" mkdir step before Package in both jobs; also dropped mac.binaries whisper-cli (ad-hoc signing walked it → codesign "No such file"). Voice excluded from CI builds per user. RESULT: build-win + build-mac BOTH GREEN, installers built (run 29404066249). Only `release` step fails: RELEASES_TOKEN = "Bad credentials" (user must refresh fine-grained PAT, Contents:RW on adexdsamson/aria-releases). Installers still downloadable as run artifacts | 2026-07-15 | b7f06f7 / 678043c | Both build ✓ |
| 260908-kiy | Fix dual-build node-gyp resolution — postinstall hardcoded the hoisted `node_modules/node-gyp/bin/node-gyp.js`, but node-gyp is only a transitive dep of @electron/rebuild, so a clean `pnpm install` (which cleared the legacy hoist artifact) broke the Node-ABI half of the dual build with MODULE_NOT_FOUND and failed postinstall (exit 1). Electron half still succeeded, so the app ran but vitest's ABI variant was missing. Now `resolveNodeGyp()` tries three tiers declared-dependency-first: direct require.resolve -> @electron/rebuild main entry (its `exports` map blocks ./package.json) -> `.pnpm/node-gyp@*` scan last, so a planted higher version can't pre-empt the dep graph. Verified: tier 2 won (node-gyp 12.4.0), `rebuild:native:node` green, both variants in aria-abi/ (ABI stamp 127, host Node 22.23.0), active binding left on the Electron variant. No vitest run. | 2026-09-08 | eb59cc6 | Verified ✓ |
| 260908-l5m | Make bench run under plain Node by swapping the ABI binding. `pnpm bench*` died at buildCorpus() with NODE_MODULE_VERSION 145 vs 127 - the bench harness runs under tsx (plain Node) but postinstall deliberately leaves the Electron-ABI binding active for the desktop app. Extracted the vitest globalSetup swap logic into a shared `scripts/native-abi.mjs` (onLocked policy: warn default for vitest, throw for bench) plus `scripts/with-node-abi.mjs`, an out-of-process runner. The wrapper is essential, not stylistic: measured on Windows, a process that has mapped better_sqlite3.node can NEVER overwrite it (copyFileSync, buffer-write and rename-aside all fail EBUSY/EPERM), so an in-process finally would have failed to restore on every run - turning "app stranded on wrong ABI" from occasional into guaranteed. Restore is triggered from finally + SIGINT/SIGTERM + process exit, asserted by sha256, and a failed restore forces a non-zero exit with a banner naming `pnpm rebuild:native:electron`. Verified: bench (22s) / bench:quick (7s) / bench:live (30s) all exit 0 and all end on the Electron variant; restore never once hit a lock. vitest NOT run (V-4). package.json bench rewiring left UNCOMMITTED with the developer bench WIP (helper-first). | 2026-09-08 | af4e72c / 759d53f / 38ae44d | Verified (Ctrl-C: finally-path proven; console-signal probe inconclusive) |
