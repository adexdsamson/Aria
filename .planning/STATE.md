...
| 260715-pkg | Unblock CI packaging (broken since 2026-06-28, blocking OAuth UAT installer). electron-builder 26 rejected invalid `platform` key on 2 whisper extraResources entries at schema validation. Relocated whisper to win/mac extraResources blocks; added "Stage voice binaries dir" mkdir step before Package in both jobs (whisper gitignored/absent in CI → voice excluded from CI builds per user). YAML + JSON validated | 2026-07-15 | (pending) | Needs Review |
