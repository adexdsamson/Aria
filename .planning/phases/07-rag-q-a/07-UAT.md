...
result: pass-with-caveats — Gaps 1-5 closed (commits 0891d9b, f356322, d936ca8, e7c0bac, 4db35c3). Phase 7 targeted suite now 140 pass / 18 fail / 1 skip (was 63 / 95 / 1). Remaining 18: 1 Ollama precondition (Gap 6 — requires OLLAMA_AVAILABLE=1) + 17 `app_meta` schema collision in people-directory / person-resolver / backfill / hybrid-retrieval tests (Phase 7 implementation uses `(key, value)` columns while migration 001 ships `(k, v)`; out-of-scope test-infra gap-closure, logged for Phase 7 followup). No previously-green test turned red.
...
**Closure status (2026-05-19):** Gaps 1-5 CLOSED via commits 0891d9b, f356322, d936ca8, e7c0bac, 4db35c3. Targeted suite went from 63/95/1 → 140/18/1. Remaining 18 failures: 1 Ollama precondition (Gap 6 — by design) + 17 `app_meta` schema collision in Phase 7 source (out-of-scope; new Phase 7 modules write to `app_meta(key, value)` while migration 001 ships `app_meta(k, v)`). The 17 failures are a Phase 7 implementation bug, not a test-infra gap — logged as separate followup.
...
### Gap 7 — `app_meta` schema collision in Phase 7 modules (HIGH — discovered during Gap 1-5 closure)
After Gaps 1-5 closed, 17 tests across `people-directory.test.ts`, `person-resolver.test.ts`, `backfill.test.ts`, `hybrid-retrieval.test.ts` now reach the DB and fail with `SqliteError: no such column: value` / `table app_meta has no column named key`.
Root cause: migration `001_init.sql` ships `CREATE TABLE app_meta(k TEXT PRIMARY KEY, v TEXT NOT NULL)`, but Phase 7 modules (`src/main/rag/people-directory.ts`, etc.) write/read via `app_meta(key, value)`. The `CREATE TABLE IF NOT EXISTS app_meta(key …)` guard in those modules is a no-op against the already-migrated table.
**Fix:** Either (a) rewrite Phase 7 callers to use `(k, v)`, or (b) add a migration that adds `key`/`value` columns / a separate `rag_meta` table. Out of scope for the test-infra gap-closure pass; routed to Phase 7 followup.
**Blast:** 17 of the 18 remaining failures in the targeted suite.
