...
|----|-------------|----------------------|-----------|----------|
| SC1 voice drives triage/schedule/draft/ask via SAME in-process service | VOICE-09 | voice-intent-router parses → calls the exported service fn (draftReply/proposeCalendarChange/summarizeThread/ask-service/pushApprovedMeetingActions); performAsk() LOCAL/FRONTIER/fallback/writeLog paths directly asserted; ask.spec.ts passes UNCHANGED after extraction | unit (ask-service.spec T1 + router + ask.spec T2) + integration | 17-02 T1: `tests/unit/main/rag/ask-service.spec.ts` green (LOCAL route, FRONTIER route, classifyFrontierError fallback, writeLog called once); 17-02 T2: `ask.spec.ts` green unmodified; router spec asserts dispatch to each service |
