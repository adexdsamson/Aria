...
`,
  ...
  {
    version: 132,
    file: '132_knowledge_folders.sql',
    sql: `
...
`,
  ...
  {
    version: 135,
    file: '135_repair_approval_child_fks.sql',
    sql: `-- Migration 135: repair child-table foreign keys left dangling by migration 124.
--
...
`,
  ...
  {
    version: 136,
    file: '136_voice_latency_log.sql',
    sql: `-- Migration 136: voice_latency_log table for D-06 per-stage latency telemetry.
--
...
`,
  ...
  {
    version: 137,
    file: '137_approval_cancelled_state.sql',
    sql: `PRAGMA foreign_keys=OFF;
-- legacy_alter_table=ON makes RENAME behave as in SQLite < 3.25: it does NOT
-- rewrite references to \`approval\` inside OTHER objects (child-table foreign
-- keys, views, triggers). Without this, RENAME approval -> approval_old silently
...
CREATE TABLE approval (
  ...
  state TEXT NOT NULL CHECK (state IN (
    'pending','generating','ready','approved','rejected','snoozed','interrupted','sent',
    'sending','failed','needs-operator-decision',
    'cancelled'   -- NEW: Phase 17 D-11 voice-path abort (distinct from rejected)
  )),
  created_at TEXT NOT NULL,
  updated_at TEXT NOT NULL,
  approval_path TEXT NOT NULL DEFAULT 'explicit' CHECK (approval_path IN ('explicit','silent','voice-explicit')),
...
`,
  ...
  {
    version: 138,
    file: '138_whatsapp.sql',
    sql: `-- Migration 138: WhatsApp foundation — 4 new tables + provider_account CHECK rebuild.
--
...
-- Assumption A2 LOCKED: 4-col composite PK (type, key_id).
-- Every keys.set() batch MUST use a single db.transaction() (gate 4).
-- Column is named 'type' (not 'key_type') to match auth-state.spec.ts raw SQL queries.
