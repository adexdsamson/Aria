...
describe('package.json build config', () => {
  ...
  // Quick task 260602-e4h — brand-icon wiring. Without per-platform `icon`
  // keys + committed raster assets, electron-builder embeds the default
  // Electron logo. These guard against silent regression of that fix.
  it('Test 8 — per-platform app icons point at generated raster assets', () => {
    const mac = build.mac as Record<string, unknown>;
    const win = build.win as Record<string, unknown>;
  ...
  it('Test 9 — icon.ico + icon.png committed and copied into resources via extraResources', () => {
    // Runtime BrowserWindow icon (resolveBrandIcon) reads these from
    // process.resourcesPath in packaged builds.
    const extra = (build.extraResources ?? []) as Array<{ filter?: string[] }>;
    const filters = extra.flatMap((e) => e.filter ?? []);
    expect(filters).toContain('icon.ico');
    expect(filters).toContain('icon.png');
    // Assets must exist on disk (generated by `npm run icons:app`, committed).
    expect(() => readFileSync(resolve(process.cwd(), 'build/icon.ico'))).not.toThrow();
    expect(() => readFileSync(resolve(process.cwd(), 'build/icon.png'))).not.toThrow();
  });
});
