...
**Core Value:** A trustworthy daily briefing + insights layer is the wedge; chief-of-staff actions (email, calendar, meeting follow-through) are the value compound. If Aria tells the user what matters every morning and has handled the rest by Friday, it has earned its place.
...
- **Team:** Solo dev + Claude Code. Phases must be small enough that one person can complete them in a session.
